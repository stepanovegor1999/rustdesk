...
Version: %s
Architecture: %s
Maintainer: RustDesk Gubernia <support@tv-gubernia.ru>
Homepage: https://tv-gubernia.ru
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service "./build/macos/Build/Products/Release/RustDesk Gubernia.app/Contents/MacOS/"')
    '''
    system2(
        "create-dmg --volname \"RustDesk Gubernia Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon \"RustDesk Gubernia.app\" 200 190 --hide-extension \"RustDesk Gubernia.app\" rustdesk-gubernia.dmg \"./build/macos/Build/Products/Release/RustDesk Gubernia.app\"")
    os.rename("rustdesk-gubernia.dmg", f"../rustdesk-gubernia-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e "../../{flutter_build_dir_2}/Gubernia Desktop.exe"')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rustdesk.exe "target/release/RustDesk Gubernia.exe"')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                '"target\\release\\RustDesk Gubernia.exe"')
        else:
            print('Not signed')
        system2(
            f'cp -rf "target/release/RustDesk Gubernia.exe" {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip "target/release/bundle/osx/RustDesk Gubernia.app/Contents/MacOS/rustdesk"')
                system2(
                    'cp libsciter.dylib "target/release/bundle/osx/RustDesk Gubernia.app/Contents/MacOS/"')
    ...
    codesign -s "Developer ID Application: {0}" --force --options runtime  "./target/release/bundle/osx/RustDesk Gubernia.app/Contents/MacOS/"*
    codesign -s "Developer ID Application: {0}" --force --options runtime  "./target/release/bundle/osx/RustDesk Gubernia.app"
    '''.format(pa))
                system2(
                    'create-dmg "RustDesk Gubernia %s.dmg" "target/release/bundle/osx/RustDesk Gubernia.app"' % version)
                os.rename('RustDesk Gubernia %s.dmg' %
                          version, 'rustdesk-gubernia-%s.dmg' % version)
                if pa:
                    system2('''
