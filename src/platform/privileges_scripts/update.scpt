on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/ru.tvgubernia.rustdesk_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/ru.tvgubernia.rustdesk_service.plist"
  set app_bundle to "/Applications/RustDesk Gubernia.app"
  ...
  set kill_others to "pids=$(pgrep -x 'RustDesk Gubernia' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "RustDesk Gubernia wants to update itself" with administrator privileges
end run
